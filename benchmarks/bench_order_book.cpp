...
static void BM_OrderBookInsert(benchmark::State& state){
    for (auto _ : state) {
        ...
        for (int i = 0; i < 1000; i++) {
            book.addBid(10000 + i, 10);
        }
        benchmark::DoNotOptimize(book);
    }
}

static void BM_OrderBookAsk(benchmark::State& state){
    OrderBook book;

    for (auto _ : state){
        book.addAsk(10001, 10);
        benchmark::DoNotOptimize(book);
        benchmark::ClobberMemory();
    }
}

static void BM_OrderBookRandom(benchmark::State& state){
    OrderBook book;
