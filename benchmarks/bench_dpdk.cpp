...
static void BM_DPDKPacketProcess(benchmark::State& state){
    ...
    for(auto _ : state){
        ...
        if(rb->pop(consumed)){
            uint8_t* data = rte_pktmbuf_mtod(
                consumed,
                uint8_t*
            );
        
            uint64_t price = *reinterpret_cast<uint64_t*>(data);
...
BENCHMARK(BM_DPDKPacketProcess);